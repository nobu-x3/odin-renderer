...
image_create :: proc(ctx: ^Context, using image_info: ^ImageInfo) -> Image{
    ...
    image_ci.samples = {._1}
    image_ci.sharingMode = .EXCLUSIVE
    image.width = width
    image.height = height
    if res := vk.CreateImage(ctx.device, &image_ci, nil, &image.handle); res != .SUCCESS{
    ...
    if res := vk.AllocateMemory(ctx.device, &mem_alloc_info, nil, &image.memory); res != .SUCCESS{
        log.fatal("Failed to allocate image memory.")
        os.exit(1)
    }
    if res := vk.BindImageMemory(ctx.device, image.handle, image.memory, 0); res != .SUCCESS{
        log.fatal("Failed to bind image memory.")
        os.exit(1)
    }
    if create_view {
        image.view = image_view_create(ctx, format, view_aspect_flags)
    }
    return image
}
...
image_view_create :: proc(ctx: ^Context, format: vk.Format, aspect_flags: vk.ImageAspectFlags) -> vk.ImageView {
    image_view : vk.ImageView
    view_ci : vk.ImageViewCreateInfo
    view_ci.sType = .IMAGE_VIEW_CREATE_INFO
    view_ci.viewType = .D2
    view_ci.format = format
    view_ci.subresourceRange = {baseMipLevel = 0, levelCount = 0, baseArrayLayer = 0, layerCount = 0}
    if res := vk.CreateImageView(ctx.device, &view_ci, nil, &image_view); res != .SUCCESS{
        log.fatal("Failed to create image.")
        os.exit(1)
    }
    return image_view
